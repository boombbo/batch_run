...
class Backend(metaclass=abc.ABCMeta):
    ...
    def commit_object(cls,
        ...
        objects, the `relative_path` may become handy to compose the committed object's address within the data source's
        interface.
        ...
        :param relative_path: List of idShort strings to resolve the `committed_object` starting at the `store_object`,
            such that `obj = store_object; for i in relative_path: obj = obj.get_referable(i)` resolves to the
            `committed_object`. In case that `store_object is committed_object`, it is an empty list.
    ...
    def update_object(cls,
        ...
        :param store_object: The object which originates from the relevant data source (i.e. has the relevant source
            attribute). It may be the `committed_object` or one of its ancestors in the AAS object hierarchy.
        :param relative_path: List of idShort strings to resolve the `updated_object` starting at the `store_object`,
            such that `obj = store_object; for i in relative_path: obj = obj.get_referable(i)` resolves to the
            `updated_object`. In case that `store_object is updated_object`, it is an empty list.
        :raises BackendNotAvailableException: when the external data source cannot be reached
        """
        pass
